...
const createRacer = require('./racer')
...
class InterfaceMonitor extends EventEmitter {
  constructor () {
    ...
    const acquireLock = () => 
      net.createServer(c => c.on('error', () => {}))
        .on('error', err => console.log(err.message) || setTimeout(acquireLock, 1000))
        .listen(8964, () => this.activate())
  ...
  ifup (iface) {
    ...
    const n3 = Math.floor(Math.random() * 155) + 100
    const n4 = Math.floor(Math.random() * 155) + 100
